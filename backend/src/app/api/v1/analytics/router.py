...
from app.api.v1.academic.schemas import StudentCardSchema, PerformanceStatsSchema
...
async def get_student_card(student_id: int, service: Annotated[PerformanceAnalyticsService, Depends(get_performance_analytics_service)]) -> StudentCardSchema:
...
async def print_student_card(student_id: int, service: Annotated[PerformanceAnalyticsService, Depends(get_performance_analytics_service)]):
    ...
    html_content = f"""<!DOCTYPE html><html><head><title>Карточка студента</title><style>body{{font-family:Arial,sans-serif;margin:20px}}h1{{text-align:center}}table{{width:100%;border-collapse:collapse;margin:20px 0}}th,td{{border:1px solid #ddd;padding:8px;text-align:left}}th{{background-color:#4CAF50;color:white}}tr:nth-child(even){{background-color:#f2f2f2}}@media print{{body{{margin:0}}}}</style></head><body><h1>Карточка успеваемости студента</h1><p><strong>ФИО:</strong> {result['student']['last_name']} {result['student']['first_name']} {result['student']['middle_name'] or ''}</p><p><strong>Дата рождения:</strong> {result['student']['birth_date']}</p><p><strong>Год поступления:</strong> {result['student']['enrollment_year']}</p><p><strong>Группа:</strong> {result['study_group'] or 'Не указана'}</p><h2>Оценки по семестрам</h2>"""
